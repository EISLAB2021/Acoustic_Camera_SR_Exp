...
def img_to_tensor(ACimages, convert_bgr_to_rgb=True, to_float32=True):
    ...
    Acoustic camera images are typically displayed in pseudo-color.
    Args:
        ACimages (numpy.ndarray or list): Input pseudo-color acoustic camera (AC) image or list of images.
        convert_bgr_to_rgb (bool): Convert BGR to RGB format if True.
        For better display, pseudo-color mode is used in this part.
        to_float32 (bool): Convert ACimage data to float32.
    ...
    def _to_tensor(image, convert_bgr_to_rgb, to_float32):
        if image.shape[2] == 3 and convert_bgr_to_rgb:
            image = cv2.cvtColor(image, cv2.COLOR_BGR2RGB)  # pseudo-color acoustic camera images
        if to_float32:
...
def generate_speckle_noise(ACimage, shape_parameter=0.4, scale_parameter=0.6):
    """
    Generate random speckle noise and add it to the acoustic camera image.
    Simply simulate speckle noise,intensity and mode can be customized.
...
class AcousticCameraDatadegration:
    def __init__(self, img_path):
        ...
        This pipeline in refer to RealESRGANDatadegration.
        Args:
    ...
    def load_image(self):
        ...
        Returns:
            torch.Tensor: Preprocessed Acoustic image tensor.
            str: Image path.
        """
        gt_path = self.path
        img_gt = cv2.imread(gt_path, cv2.IMREAD_COLOR)  # load pseudo-color acoustic image
    ...
    def synthesis(self):
        """
        Perform LQ-image synthesis with noise and JPEG degradation.

        Returns:
            LQ: Degraded low quality (LQ) acoustic camera image tensor.
            The LQ image can be further downsampled to obtain the LR image.
        """
        ...
        LQ_ACimage = torch.clamp((img_gt * 255.0).round(), 0, 255) / 255.0
        LQ_ACimage = LQ_ACimage.squeeze(0).contiguous()
...
if __name__ == '__main__':
    LQ_ACimg = AcousticCameraDatadegration('ACimage.jpg')  # Acoustic camera input
    result = LQ_ACimg.synthesis()
    # The LQ image can be further downsampled to obtain the LR image.

    # Save the processed image
    torchvision.utils.save_image(result, 'degraded_LQ_ACimage.jpg')

    # Visualizing statistical feature maps on degraded LQ acoustic camera images
    # Read the processed image
    processed_image = cv2.imread('degraded_LQ_ACimage.jpg')
    ...
    # Save the entire LQ acoustic camera image analysis result
    plt.savefig('LQ_ACimage_analysis.png')

    # Optionally, display the LQ image analysis result
    plt.show()
